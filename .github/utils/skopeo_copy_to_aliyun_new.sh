...
do
    ...
    if [[ $skopeo_flag -eq 0 ]]; then
        echo "$(tput -T xterm setaf 1)skopeo copy $REGISTRY/$image to infracreate-registry.cn-zhangjiakou.cr.aliyuncs.com/apecloud/$image_name error$(tput -T xterm sgr0)"
        exit 1
    fi
done < $FILE_NAME
